...
Release candidate: iOS `0.1.1` / build `1`
...
| Daily content request | One HTTPS `GET /v1/daily/YYYY-MM-DD` request. The app adds only an `Accept: application/json` header and sends no selected sign, saved card, birth data, account identifier, advertising identifier, or location. | The requested date is used to return the edition. Krazel does not retain or associate connection data with a user. | Apple's real-time request exception supports `No data collected`. |
...
| StoreKit | Apple StoreKit is present to load product identifiers and verified entitlements. Version `0.1.1` has no active App Store Connect IAP or subscription products, so no purchase can be made. | No payment details are received or stored by Krazel. Re-audit before activating a product. | Exact review-note disclosure; not a current App Privacy collection. |
...
- App Review contact details are filled only in Apple's private required review
  ...
  any public page.
- Review notes now describe the exact `0.1.1` build: no login, no permissions,
  no ads/analytics, date-only content request, local saves, offline fallback,
  and inactive StoreKit products. They contain no infrastructure name or future
  promise.
...
This audit passes for `0.1.1` / build `1`. Before submission, verify the selected
