...
- The current FreeAstro editorial template is rendered directly in native
  Spanish. Its sign, planet, house, element, priority, advice, area, phase,
  color, headline, and keywords are mapped deterministically, so the normal
  daily path is free and cannot invent or omit provider facts. If FreeAstro
  introduces an unknown template, `@cf/openai/gpt-oss-20b` remains a guarded
  fallback and `@cf/meta/llama-3.1-8b-instruct-fast` reviews its fidelity.
- Scores, lucky number, source identity, sign, date, and content version are
  ...
  app contract.
- Spanish cache revision 8 invalidates the old or insufficiently reviewed translations without
