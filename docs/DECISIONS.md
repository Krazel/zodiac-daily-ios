...
- Each sign is now adapted in one structured request with the free-plan-capable
  ...
  or facts. A separate `@cf/openai/gpt-oss-20b` bilingual review must approve
  ...
  be cached. A local deterministic gate rejects known agreement errors and
  untranslated provider vocabulary before the model review.
- Scores, lucky number, source identity, sign, date, and content version are
  ...
  app contract.
- Spanish cache revision 5 invalidates the old or insufficiently reviewed translations without
