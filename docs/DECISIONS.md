...
- Each sign is now adapted in one structured request with the free-plan-capable
  ...
  or facts. A separate `@cf/meta/llama-3.1-8b-instruct-fast` bilingual review must approve
  fidelity, grammar, natural Spanish, complete metadata translation,
  astrological meaning, and absence of invented facts before the edition can
  be cached. A deterministic normalization layer corrects known agreement
  errors and supplies canonical Spanish for the provider's finite signs,
  phases, colors, headlines, and keyword vocabulary before the model review.
- Scores, lucky number, source identity, sign, date, and content version are
  ...
  app contract.
- Spanish cache revision 7 invalidates the old or insufficiently reviewed translations without
